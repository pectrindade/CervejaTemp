...
namespace CervejaTemp.Forms
{
    public partial class CadAdjunto : Form
    {
        ...
        private void CadAdjunto_Load(object sender, EventArgs e)
        {
        ...
        private void CadAdjunto_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void BuscaCodigo(int codigo)
        {
            ...
            var dr = Classes.Mysql.Adjunto.Select(codigo);
            ...
            if (dr.HasRows)
            {
                while (dr.Read())
                {
                    ...
                    txtcodigo.Text = dr.GetString(dr.GetOrdinal("CODADJUNTO"));
                    txtdatacadastro.Text = dr.GetString(dr.GetOrdinal("DATAENTRADA"));
                    txtDescricao.Text = dr.GetString(dr.GetOrdinal("NOME"));
        ...
        private void BuscaGrid()
        {
            ...
            var dr = Classes.Mysql.Adjunto.Select();

            if (dr.HasRows)
            {
                while (dr.Read())
                {
                    if (!dr.IsDBNull(dr.GetOrdinal("CODADJUNTO"))) { linhaDados[0] = dr.GetString(dr.GetOrdinal("CODADJUNTO")); }
        ...
        private void Gravar(bool novo, int codigo)
        {
            ...
            try
            {
                var m = new Classes.Mysql.Adjunto(codigo, nome, quantidade, preco, dtfabricacao, dtvencimento, dtcadastro, ebc, obs);
                if (novo)
                    m.Insert();
                else
                    m.Update();
        ...
        private void Relatorio()
        {
            ...
            if (dr.HasRows)
            {
                while (dr.Read())
                {
                    var cod = dr.GetString(dr.GetOrdinal("CODADJUNTO"));
                    var nome = dr.GetString(dr.GetOrdinal("DESCRICAO"));

                    try
                    {
                        //var m = new Classes.Mysql.Adjunto(int.Parse(cod), nome);
